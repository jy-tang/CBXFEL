...
for k in range(Nshot_total):
    ...
    jobid, sim_name = start_simulation(folder_name = folder_name, dKbyK = 0.03,undKs = 1.172,und_period = 0.026,und_nperiods=130, nslice = nslice, zsep = zsep, dgrid = dgrid, ncar = ncar,
                                           nametag = nametag,gamma0 = np.around(8000./0.511,3), 
                                           Nf=4, Nt=28, emitnx = 0.3e-6, emitny = 0.3e-6,
                                           pulseLen = pulseLen, sigma = sigma, chirp = chirp, Ipeak = Ipeak,  xlamds = xlamds,
