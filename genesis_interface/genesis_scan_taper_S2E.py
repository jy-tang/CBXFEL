...
def start_simulation(dKbyK, folder_name, undKs = 1.169, und_period = 0.026, und_nperiods=130, ipseed=0, waitQ = False, verboseQ = True, nametag = '',gamma0 = np.around(8000./0.511,3), emitnx = 0.3e-6, emitny = 0.3e-6, betax = 10.0, betay=10.0, alphax = 0.0, alphay = 0.0, pulseLen = 100e-15, sigma = 10e-15, chirp = 10, Ipeak = 2e3, distfile = None ):
    
    root_dir = os.path.realpath(os.path.curdir)
    cwd =root_dir + '/' + folder_name
    os.system('cp  genesis_interface/make_lattice2.py ' + cwd)
    os.system('cp  genesis_interface/genesismp_s2e.in ' + cwd)
    os.system('cp  genesis_interface/RunGenesis2.sh ' + cwd)
    os.system('cp ' + distfile + ' ' + cwd)
   ...
   # g.input['xlamds'] = 1.76363e-09
    ...
    g.input['rxbeam'] = np.sqrt(betax*emitnx/gamma0)
    g.input['rybeam'] = np.sqrt(betay*emitny/gamma0)
    
    print("initial beam size",np.sqrt(betax*emitnx/gamma0), np.sqrt(betay*emitny/gamma0) )
    
    g.input['emitx'] = emitnx
    g.input['emity'] = emitny
    g.input['alphax'] = alphax
    g.input['alphay'] = alphay
...
start_simulation(dKbyK, folder_name, undKs = 0.7, und_period = 0.026, und_nperiods=130, ipseed=0, waitQ = False, verboseQ = True, nametag = '',gamma0 = np.around(2550./0.511,3), emitnx = 0.075e-6, emitny = 0.075e-6, betax = 10.0, betay=10.0, alphax = 2.0, alphay = 2.0, pulseLen = 100e-15, sigma = 10e-15, chirp = 10, Ipeak = 2e3, distfile = None)

#for taper in np.linspace(0.05, 0.05, 1):
#    start_simulation(folder_name = 'data_S2E_0_22', dKbyK = taper,undKs = 1.169,und_period = 0.026,und_nperiods=130, pulseLen =60e-15, sigma = 20e-15, chirp = 0, nametag ='h',gamma0 = np.around(8009/0.511,6), Nf=3, Nt = 29,Ipeak = 2e3, emitnx = 0.3e-6, emitny = 0.3e-6, ipseed = np.random.randint(1000), distfile = '/sdf/group/beamphysics/jytang/genesis/CBXFEL/XAMP_0.24/HXRSTART_matchproj.dist')

#for undK in np.linspace(1.165, 1.167, 3):
#    start_simulation(folder_name = 'data_S2E_0_22', dKbyK =  0.0,undKs = undK,und_period = 0.026,und_nperiods=130, pulseLen =60e-15, sigma = 20e-15, chirp = 0, nametag ='t',gamma0 = np.around(8000/0.511,6), Nf=2, Nt = 5,Ipeak = 2e3, emitnx = 0.3e-6, emitny = 0.3e-6, ipseed = np.random.randint(1000), distfile = '/sdf/group/beamphysics/jytang/genesis/CBXFEL/XAMP_0.22/HXRSTART_matchproj.dist')
