...
def start_simulation(dKbyK, folder_name, undKs = [1.169]*40, und_period = 0.026, und_nperiods=130, quad_grad=12.64, ipseed=0, waitQ = False, verboseQ = True, nametag = '',gamma0 = np.around(8000./0.511,3), Nf = 5, Nt = 27, emitnx = 0.3e-6, emitny = 0.3e-6,pulseLen = 100e-15, sigma = 10e-15, chirp = 10, Ipeak = 2e3 ):
    ...
    sim_name = 'tap'+str(np.around(dKbyK,6))+'_K'+str(np.around(undKs[-1],6))
    ...
    #xrms_match, yrms_match = match_to_FODO(gamma0 = gamma0, emitnx = emitnx, emitny = emitny, L_quad=0.3)
    xrms_match, yrms_match = match_to_FODO(gamma0 = gamma0, emitnx = emitnx, emitny = emitny, L_quad=0.26, L_drift=3.9, g_quad=quad_grad)
    #---------------------Genesis Input-------------------------------------------#
   ...
   # g.input['xlamds'] = 1.76363e-09
    ...
    g.input['nslice'] = 1024#3865 #0 # have genesis make beam time-slices from the input particle distribution
    ...
    g.input['fieldfile'] = 'n6_seed.dfl3'
...
chirp = 0
start_simulation(folder_name = 'data_3GeV_harm', dKbyK = 0,undKs = [1.172]*40 ,und_period = 0.026,und_nperiods=130, pulseLen =80e-15, sigma = 20e-15, chirp = chirp, nametag ='b',gamma0 = np.around(8000./0.511,3), Nf=1, Nt = 11,Ipeak = 2e3, emitnx = 0.3e-6, emitny = 0.3e-6, quad_grad = 10)
