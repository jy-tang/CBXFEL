...
def start_simulation(dKbyK, folder_name, undKs = 1.169, und_period = 0.026, und_nperiods=130, ipseed=0, waitQ = False, verboseQ = True, nametag = '',gamma0 = np.around(8000./0.511,3), Nf = 5, Nt = 27, emitnx = 0.3e-6, emitny = 0.3e-6,pulseLen = 100e-15, sigma = 10e-15, chirp = 10, Ipeak = 2e3 ):
    ...
    beamfilename = 'chirp' + str(int(chirp)) +'.beam'
...
for chirp in np.linspace(40, 50, 3):
    start_simulation(folder_name = 'comp_theory', dKbyK =  0.0,undKs = 1.172,und_period = 0.026,und_nperiods=130, pulseLen =100e-15, sigma = 20e-15, chirp = chirp, nametag ='x' + str(int(chirp)),gamma0 = np.around(8000/0.511,6), Nf=2, Nt = 3,Ipeak = 2e3, emitnx = 0.3e-6, emitny = 0.3e-6, ipseed = np.random.randint(1000))
    
#for tdur in [24,30,60]:
#    start_simulation(folder_name = 'comp_theory', dKbyK =  0.0,undKs = 1.172,und_period = 0.026,und_nperiods=130, pulseLen =tdur*1e-15, sigma = 20e-15, chirp = 20, nametag ='j' + str(int(tdur)),gamma0 = np.around(8000/0.511,6), Nf=2, Nt = 3,Ipeak = 1e3, emitnx = 0.3e-6, emitny = 0.3e-6, ipseed = np.random.randint(1000))
    
    
#for chirp in [5,10,15,20,25,30]:ls
