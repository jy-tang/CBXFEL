...
def start_simulation(dKbyK, folder_name, undKs = 1.169, und_period = 0.026, und_nperiods=130, ipseed=0, waitQ = False, verboseQ = True, nametag = '',gamma0 = np.around(8000./0.511,3), Nf = 5, Nt = 27, emitnx = 0.3e-6, emitny = 0.3e-6, Ipeak = 2e3, sep = 10e-15, tdur = 20e-15,  detune1 = 0.0, detune2 = 0.0,  dgam = 0.0 ):
    ...
    sim_name = 'K'+str(np.around(undKs,6)) + '_dt' +str(int(detune1*1e4)) + '_' + str(int(detune2*1e4)) #'tap'+str(np.around(dKbyK,6))+'_K'+str(np.around(undKs,6))
    ...
    c_speed = 299792458
    N = 100
    ts = np.linspace(0, 2*tdur + sep, N)
    zs = ts*c_speed
    ind1 = (ts< tdur) &(ts > 0)
    ind2 = (ts  > tdur + sep) &(ts < 2*tdur + 2)
    
 
    gamma = gamma0*np.ones((N,))
    gamma[ind1] *= (1 + detune1)
    gamma[ind2] *= (1 + detune2)
        
    I = np.zeros((N,))
    I[ind1] = Ipeak
    I[ind2] = Ipeak

    delgam = np.ones((N,))*dgam /0.511
    enx = np.ones((N,))*emitnx
    eny = np.ones((N,))*emitny
    beamfile={'ZPOS':zs,'CURPEAK':I, 'GAMMA0': gamma, 'DELGAM':delgam, 'EMITX':enx, 'EMITY':eny}
    beamfilename = 'detune' + str(int(detune1*1e4)) + '_' + str(int(detune2*1e4)) +'.beam'
...
for i in range(0, 5):
    start_simulation(folder_name = 'data_phaselock', dKbyK =  0.0,undKs = 1.172,und_period = 0.026,und_nperiods=130,  Ipeak = 2e3, nametag ='f' + str(i), gamma0 = np.around(8000/0.511,6), Nf=2, Nt = 7, emitnx = 0.3e-6, emitny = 0.3e-6, ipseed = np.random.randint(1000), sep = 50e-15, tdur = 20e-15,  detune1 = 0.000*i, detune2 = 0.0001*i,  dgam = 0.0 )
