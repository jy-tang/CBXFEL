...
for k in range(Nshot_total):
    ...
    if k == 0:
        with open(folder_name + '/beam_info.txt', "w") as myfile:
            myfile.write("bunch_length(fs) energy_chirp(MeV) \n")
    ...
    if nEbeam_flat_init > 0 and  nEbeam_flat > 0 and (k < nEbeam_flat_init or (k - nEbeam_flat_init) %nEbeam_flat != 0):
        ...
        chirp = beam_chirp*(1+ normal(scale = chirp_jitter))
        print('shot ', str(k), 'beam chirp', str(chirp))
    
    with open(folder_name + '/beam_info.txt', "a") as myfile:
        beam_info = [pulseLen, chirp]
        myfile.write(" ".join(str(item) for item in beam_info))
        myfile.write("\n")
    ...
    jobid, sim_name = start_simulation(folder_name = folder_name, dKbyK = 0.03,undKs = 1.172,und_period = 0.026,und_nperiods=130, nslice = nslice, zsep = zsep,
                                           ...
                                           pulseLen = pulseLen, sigma = 20e-15, chirp = chirp, Ipeak = 2e3,
                                           xlamds = xlamds,
